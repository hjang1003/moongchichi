        ...
        if blocked_entities:
            ...
            if soft_lines:
                system_prompt += (
                    ...
                    "[뭉뚱그리기 금지] 여기 있는 이름을 피하려고 '글로벌 플랫폼', '해외 업체', "
                    "'한 대형 브랜드', '업계 관계자' 같은 익명 표현으로 바꾸지 마라. "
                    "어느 회사 얘기인지 알 수 없으면 브리핑의 값이 사라진다. "
                    "목록에 없는 다른 고유명사를 찾아 구체적으로 쓰고, 그런 사례를 못 찾으면 "
                    "그 소재를 버리고 다른 소재를 골라라:\n"
                    + "\n".join(soft_lines)
                    # 목록이 길어 앞의 규칙이 파묻힌다. 실제로 이 규칙을 어긴 사례가 있어 끝에서 한 번 더 못박는다.
                    + "\n위 이름들에 대해 다시 확인한다. 한 항목에서 두 문장 이상 쓰지 마라. "
                    "이미 인용했던 실적·수치를 다시 인용하지 마라. 출처로 쓰는 것은 제한 없다."
                )

        if recent_titles:
            system_prompt += (
                "\n\n[최근 다룬 항목 — 반복 금지]\n"
                "아래는 최근 2주간 이미 내보낸 항목이다. '제목 :: 핵심 문단' 형식이다.\n"
                "\n"
                "1. 같은 수치·사례 재인용 금지. 핵심 문단에 이미 나온 통계와 사례를 "
                "다시 쓰지 마라. 출처 기사가 다르더라도 같은 숫자면 같은 내용이다. "
                "다른 기사를 찾았는데 알맹이가 이미 쓴 것과 같다면 그 소재를 버려라.\n"
                "2. 같은 구도 반복 금지. 고유명사가 달라도 'A 브랜드의 B 콘텐츠 성과/전략' 같은 "
                "동일 문형을 다시 쓰지 마라. 실제로 '홈·리빙 브랜드의 콘텐츠 전략'류가 "
                "9주 연속 나간 적이 있다.\n"
                "3. 같은 업종 연속 금지. 아래 목록에서 특정 업종(패션, 음악, 홈리빙, IT·테크 등)이 "
                "최근 3일 연속 등장했다면 오늘은 그 업종을 빼라. "
                "실제로 패션이 10일 중 9일 연속 나온 적이 있다. "
                "업종이 겹치면 각도를 바꾸고, 각도까지 겹치면 소재를 버려라.\n"
                "\n"
...
이 비율은 하루 기준이 아니라 최근 2주 전체 기준이다. 하루 안에서 비율을 맞추려 하지 마라.
오늘 관심 업종이 0개여도 되고 3개여도 된다. 2주를 합쳐서 대략 맞으면 된다.
매일 비율을 맞추려 들면 같은 업종이 매일 나오고, 그 업종의 소재가 금방 바닥나 같은 기사를 반복하게 된다.
...
간결하고 실무적으로 도움이 되게 작성해 주세요."""
    ...
    async def extract_entities(self, briefing_text: str) -> List[str]:
        ...
        prompt = (
            ...
            "- 인물: 마케팅 학자, 저자, 경영진 등 실존 인물 이름. 회사명은 인물이 아닙니다\n"
            "- 이론: 이름이 붙은 프레임워크·모델·개념만. 예: 4P, AARRR, 퍼미션 마케팅\n"
            ...
            "[반드시 지킬 제외 규칙]\n"
            "1. 출처로 인용된 매체·리서치 기관·분석도구 제공사는 브랜드로 추출하지 마세요. "
            "예: Warc, Nielsen, Adweek, eMarketer, IAB, Digiday, Forrester, Emplifi, "
            "Search Engine Land, 오픈서베이, 마케팅위크. "
            "이들은 브리핑이 인용하는 출처이지 다룬 소재가 아닙니다.\n"
            "2. 이론에는 고유한 이름이 붙은 것만 넣으세요. "
            "'B2C 콘텐츠 마케팅', 'Z세대 콘텐츠 소비 행동', '마케팅 이펙티브니스'처럼 "
            "주제를 서술한 표현은 이론이 아닙니다. 절대 넣지 마세요.\n"
            "3. 같은 대상은 한 가지 표기로만 쓰세요. 국문 표기가 널리 쓰이면 국문으로 통일하세요. "
            "'필립 코틀러'와 'Philip Kotler'를 둘 다 넣지 마세요.\n\n"
