...
class ClaudeService:
    ...
    async def generate_briefing(self, profile: Dict[str, str], theme: str, date_str: str, weekday_ko: str) -> str:
        ...
        system_prompt = (
            ...
            "말투 없이 정보 중심으로 작성합니다.\n"
            "각 항목은 반드시 다음 3단 구조로 작성하세요:\n"
            "  1) 배경 설명 — 이 소식/사례/데이터가 왜 지금 중요한지\n"
            "  2) 핵심 내용 — 구체적 수치, 전략, 사실 중심으로 상세히\n"
            "  3) 실무 적용 포인트 — 마케터 지망생이 실제로 써먹을 수 있는 인사이트\n"
            "각 항목 말미에는 [출처: ...] 형식으로 출처를 반드시 명기합니다.\n"
...
1. 독립적인 소식/사례/데이터를 최소 4개 이상 다룰 것 (많을수록 좋음)
2. 각 항목은 아래 3단 구조로 빠짐없이 작성할 것:
   ▸ 배경 설명: 이 소식이 왜 지금 중요한지, 업계 맥락 포함 (3~5문장)
   ▸ 핵심 내용: 구체적 수치·전략·사실 중심으로 상세히 (5~8문장)
   ▸ 실무 적용 포인트: 신입/취준생이 실제로 활용할 수 있는 인사이트 (3~4문장)
3. 각 항목 말미에 [출처: 기관명/미디어명] 형식으로 출처 명기
4. 국내 + 글로벌 소스 균형 있게 사용
5. 전체 분량: 6000자 이상
